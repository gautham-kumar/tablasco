...
{
    protected BeginningOfLineState(TableDataParser parserState)
    {
        super(parserState);
    }
    ...
    public ParserState parse(StreamTokenizer st) throws IOException, ParseException
    {
        ...
        while (nextState == null && st.ttype != StreamTokenizer.TT_EOF)
        {
            ...
            {
                if (nextValueIs(TableDataParser.SECTION_IDENTIFIER, st, nextToken))
                {
                    nextState = this.getParser().getSectionReaderState();
                }
                else if (nextValueIs(TableDataParser.METADATA_IDENTIFIER, st, nextToken))
                {
